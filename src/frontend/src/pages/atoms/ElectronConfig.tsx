...
const NOBLE_GAS_CORES: Record<number, string> = {
  2: "[He]",
  10: "[Ne]",
  18: "[Ar]",
  36: "[Kr]",
...
// Element symbols for mini periodic table
const ELEMENTS: {
  Z: number;
  sym: string;
  period: number;
...
function buildConfig(
  Z: number,
): { orbital: string; electrons: number; block: string }[] {
  if (EXCEPTIONS[Z]) return []; // handled separately in display
  const result: { orbital: string; electrons: number; block: string }[] = [];
  let remaining = Z;
...
function formatConfig(
  config: { orbital: string; electrons: number }[],
): string {
  return config.map((c) => `${c.orbital}${superscript(c.electrons)}`).join(" ");
}

function withNobleGasCore(
  config: { orbital: string; electrons: number; block: string }[],
  Z: number,
): string {
...
// ── Box notation (orbital filling) ───────────────────────────────────────────
interface BoxOrbital {
  label: string;
  slots: ("up" | "down" | "empty")[];
  block: string;
}

function buildBoxNotation(
  config: { orbital: string; electrons: number; block: string }[],
): BoxOrbital[] {
  return config.map((c) => {
    const cap = CAPACITY[c.block];
    const half = cap / 2;
    const slots: ("up" | "down" | "empty")[] = [];
    // Hund's rule: fill all spin-up first, then pair
...
// ── Components ────────────────────────────────────────────────────────────────
function SpinArrow({ spin }: { spin: "up" | "down" | "empty" }) {
  if (spin === "empty")
    return <span className="text-muted-foreground/20 text-base">−</span>;
  return (
    <motion.span
...
function OrbitalBox({
  ...
  animStep,
  idx,
}: { box: BoxOrbital; animStep: number; idx: number }) {
  const colors = BLOCK_COLORS[box.block];
  const half = box.slots.length / 2;
  const groups: ("up" | "down" | "empty")[][] = [];
  for (let i = 0; i < half; i++)
    ...
    >
      <div className="flex gap-1">
        {groups.map((pair, gi) => (
          ...
          >
            {pair.map((spin, si) => (
              <AnimatePresence key={`s-${si}-${spin}`}>
                {idx <= animStep ? (
                  <SpinArrow spin={spin} />
                ) : (
                  <span className="text-muted-foreground/10 text-base">−</span>
                )}
              </AnimatePresence>
            ))}
          </div>
      ...
      <span className={`text-xs font-mono ${colors.text}`}>{box.label}</span>
    </div>
  );
}
...
function MiniPeriodicTable({
  selected,
  onSelect,
}: { selected: number; onSelect: (Z: number) => void }) {
  const maxGroup = 18;
  const maxPeriod = 7;
  // Build grid: period=row, group=col
  const grid: Record<string, (typeof ELEMENTS)[0]> = {};
  ...
  return (
    <div className="space-y-2">
      <div className="overflow-x-auto">
        ...
        >
          {Array.from({ length: maxPeriod }, (_, pi) =>
            Array.from({ length: maxGroup }, (_, gi) => {
              const el = grid[`${pi + 1}-${gi + 1}`];
              // Skip f-block in main table, they go in separate rows
              if (el && el.block === "f")
                // biome-ignore lint/suspicious/noArrayIndexKey: placeholder divs for f-block elements have no data identity; pi/gi are grid position indices
                return <div key={`cell-f-${pi}-${gi}`} />;
              // biome-ignore lint/suspicious/noArrayIndexKey: empty grid cells have no data identity; pi/gi are grid position indices
              if (!el) return <div key={`cell-empty-${pi}-${gi}`} />;
              ...
              return (
                <button
                  ...
                  title={`Z=${el.Z} ${el.sym}`}
                  className={`rounded text-center transition-all duration-150 cursor-pointer ${
                    isSel
                      ? `${colors.bg} ring-2 ring-offset-1 ring-offset-card ${colors.text}`
                      : `${colors.bg} ${colors.text} opacity-70 hover:opacity-100`
                  }`}
                  style={{ padding: "2px 0", fontSize: 9 }}
                >
                  <div className="font-mono font-bold">{el.sym}</div>
                  <div
      ...
      <div className="overflow-x-auto">
        {[fRow, aRow].map((row, ri) => (
          <div
            key={ri === 0 ? "lanthanides" : "actinides"}
            style={{
              display: "flex",
              gap: 2,
              flexWrap: "wrap",
...
// ── Aufbau Builder Component ──────────────────────────────────────────────────
function AufbauBuilder() {
  const [filledElectrons, setFilledElectrons] = useState(0);
  const [targetZ, setTargetZ] = useState(26);
  const [animating, setAnimating] = useState(false);
  const maxElectrons = 36;

  const getConfigUpTo = (electrons: number) => {
    const result: { orbital: string; electrons: number; block: string }[] = [];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        >
          {ELEMENTS.filter((e) => e.Z <= 36).map((el) => (
      ...
      <div className="rounded-lg bg-muted/10 border border-border p-4">
        <h4 className="text-xs font-semibold text-foreground mb-3">
          Energy Level Diagram (Aufbau Order)
        </h4>
        <div className="space-y-2">
          {AUFBAU_ORDER.slice(0, 10).map((orb, _i) => {
            const blockType = orb.slice(-1);
            ...
            const cap = CAPACITY[blockType];
            return (
              <div key={orb} className="flex items-center gap-3">
                <span
                  className={`text-xs font-mono w-8 ${isFilled ? colors.text : "text-muted-foreground/40"}`}
                >
                  {orb}
                </span>
                <div className="flex-1 h-6 rounded border border-border/40 bg-muted/20 flex items-center px-1 gap-0.5">
                  {[
                    "1",
                    "2",
                    "3",
                    ...
                    .map((slotNum) => {
                      ...
                      const isUp = si < (cfg?.electrons ?? 0);
                      const isDown =
                        si >= cap / 2 &&
                        si <
                          cap / 2 +
                            Math.max(0, (cfg?.electrons ?? 0) - cap / 2);
                      return (
                        <div
                          key={`${orb}-slot-${slotNum}`}
      ...
      <div className="flex flex-wrap gap-2">
        {boxNotation.map((box) => (
          ...
          >
            <div className="flex gap-1">
              {box.slots.map((spin, si) => (
                <div
                  key={`${box.label}-spin-${si}`}
                  className="border border-border/40 rounded px-1 py-0.5 flex flex-col items-center"
                  style={{ minWidth: 22 }}
                >
                  <span
...
export default function ElectronConfig() {
  ...
  const shortConfig = isException
    ? EXCEPTIONS[selectedZ]
    : withNobleGasCore(config, selectedZ);
  ...
  return (
    ...
    >
      ...
      {/* Mini periodic table */}
      <SectionCard glowAccent data-ocid="electron_config.table_panel">
        <h2 className="font-display text-lg font-semibold text-foreground mb-3">
          Select an Element
        </h2>
        <div className="mb-3 flex gap-4 flex-wrap text-xs">
        ...
        <MiniPeriodicTable selected={selectedZ} onSelect={handleSelect} />
      </SectionCard>

      {/* Aufbau Builder */}
      <SectionCard data-ocid="electron_config.aufbau_builder">
        <h2 className="font-display text-lg font-semibold text-foreground mb-4">
          Aufbau Principle Builder
        </h2>
        <p className="text-sm text-muted-foreground mb-4">
          Build electron configurations step-by-step. Click "+1 Electron" to add
          electrons following Aufbau + Hund's rules, or select an element to
          auto-fill.
        </p>
        <AufbauBuilder />
      </SectionCard>
      ...
      {el && (
        <motion.div
          ...
          className="space-y-4"
        >
          <SectionCard data-ocid="electron_config.config_panel">
            <div className="flex items-start justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  {isException && (
                    <span className="rounded-full bg-orange-500/20 text-orange-400 border border-orange-500/40 px-2 py-0.5 text-xs font-semibold">
                      Aufbau Exception
                    </span>
                  )}
                </div>
                <div className="space-y-1">
                  ...
                  <div className="font-mono text-foreground text-sm bg-muted/30 rounded px-3 py-2 break-all">
                    {fullConfig}
                  </div>
                  <div className="text-sm text-muted-foreground mt-2">
                    Noble gas shorthand:
                  </div>
                  <div className="font-mono text-foreground text-sm bg-muted/30 rounded px-3 py-2">
                    {shortConfig}
                  </div>
                </div>
              </div>
              <div className="text-right space-y-1 text-sm text-muted-foreground">
                <div>
                  Period{" "}
                  <span className="text-foreground font-mono">{el.period}</span>
                </div>
                <div>
                  Group{" "}
                  <span className="text-foreground font-mono">{el.group}</span>
                </div>
                <div>
                  Block{" "}
                  <span className={`font-mono ${BLOCK_COLORS[el.block].text}`}>
                    {el.block.toUpperCase()}
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* Box notation */}
          <SectionCard data-ocid="electron_config.box_notation">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Box notation showing electron spins (↑ = spin up, ↓ = spin
                  down) with Hund's rule.
                </p>
              </div>
              <button
                type="button"
                ...
                className="rounded-lg bg-primary/20 border border-primary/50 text-primary px-4 py-1.5 text-sm font-medium transition-all hover:bg-primary/30 disabled:opacity-50"
              >
                {animating ? "Filling…" : "▶ Animate Filling"}
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {boxNotation.map((box, idx) => (
                <OrbitalBox
                  key={box.label}
                  box={box}
                  animStep={animStep}
          ...
          {/* Aufbau order */}
          <SectionCard data-ocid="electron_config.aufbau_diagram">
            ...
            <div className="flex flex-wrap gap-1.5">
              {AUFBAU_ORDER.map((orb, i) => {
                ...
                return (
                  <span
                    key={orb}
                    className={`font-mono text-xs rounded px-2 py-1 border transition-all duration-200 ${
                      isFilled
            ...
            <p className="text-xs text-muted-foreground mt-3">
              Highlighted orbitals are occupied for the selected element. Note:
              4s fills before 3d (penetration effect).
            </p>
          </SectionCard>
        </motion.div>
      )}
      ...
      >
        ...
        <p className="mt-3 text-sm text-muted-foreground mb-4">
          Several transition metals have configurations that differ from Aufbau
          predictions. Half-filled (d⁵) and fully-filled (d¹⁰) d-subshells are
          especially stable due to exchange energy effects.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {Object.entries(EXCEPTIONS).map(([Z, cfg]) => (
            <div
              key={Z}
              className="rounded-lg border border-orange-500/30 bg-orange-500/10 p-3"
            >
              <div className="font-mono text-xs text-orange-400 font-semibold mb-1">
                Z = {Z}
              </div>
              <div className="text-sm text-foreground">{cfg}</div>
            </div>
          ))}
        </div>
      </CollapsibleSection>
      ...
      >
        ...
        <div className="mt-3 space-y-3 text-sm text-muted-foreground leading-relaxed">
          <p>
            In hydrogen, energy depends only on n. In multi-electron atoms,
            electron-electron repulsion lifts the l-degeneracy: within a shell,
            s orbitals penetrate more toward the nucleus and are stabilized
            compared to p, d, f.
          </p>
          <p>
            Hartree-Fock theory solves N coupled integro-differential equations
            via the SCF procedure. Each electron sees an effective potential
            from all others. The Slater determinant ensures antisymmetry.
          </p>
          <p>
            Density Functional Theory (DFT) uses the electron density ρ(r) as
            the fundamental variable (Hohenberg-Kohn theorems). Kohn-Sham
            equations introduce auxiliary non-interacting orbitals, and
            exchange-correlation functionals (LDA, GGA, hybrid) approximate the
            many-body effects.
          </p>
